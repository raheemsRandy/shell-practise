...
if [ $userId -ne 0 ]
then 
    echo -e "$R please run this command with root access $N" &>>$Log_file
    exit 1
else
    echo -e "$G Your are running with root access $N"  &>>$Log_file
fi
...
if [ $? -ne 0 ]
then    
    echo "python3 not installed going to install"&>>$Log_file
...
if [ $? -ne 0 ]
then    
    echo "nginx not installed going to install"&>>$Log_file
